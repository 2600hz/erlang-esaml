...
add_xml_id(Xml) ->
    Xml#xmlElement{attributes = Xml#xmlElement.attributes ++ [#xmlAttribute{name = 'ID'
...
generate_authn_request(IdpURL, SP = #esaml_sp{}) ->
    generate_authn_request(IdpURL, SP, "urn:oasis:names:tc:SAML:2.0:nameid-format:persistent").

%% @doc Return an AuthnRequest as an XML element
-spec generate_authn_request(IdpURL :: string(), esaml:sp(), Format :: nameid_format()) -> #xmlElement{}.
generate_authn_request(IdpURL,
                       SP = #esaml_sp{consume_uri = ConsumeURI
                                     ,sp_sign_requests = Sign
                                     ,key = Key
    ...
    case Sign of
        true -> xmerl_dsig:sign(Xml, Key, Certificate);
        false -> add_xml_id(Xml)
    end.
...
generate_metadata(SP = #esaml_sp{org = Org
                                ,tech = Tech
                                ,sp_sign_metadata = Sign
                                ,key = Key
                                ,certificate = Certificate
                                }) ->
