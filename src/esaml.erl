...
-export([nameid_map/1, rev_nameid_map/1]).
-export([stale_time/1]).
...
nameid_map(S) when is_list(S) -> unspecified.

-spec rev_nameid_map(name_format()) -> string().
...
rev_nameid_map(persistent) -> "urn:oasis:names:tc:SAML:2.0:nameid-format:persistent";
rev_nameid_map(transient) -> "urn:oasis:names:tc:SAML:2.0:nameid-format:transient";
rev_nameid_map(unspecified) -> "urn:oasis:names:tc:SAML:2.0:nameid-format:unspecified".
...
decode_idp_metadata(Xml) ->
    ...
    esaml_util:threaduntil([
                            ...
                            ?xpath_attr("/md:EntityDescriptor/md:IDPSSODescriptor/@WantAuthnRequestsSigned",
                                        esaml_idp_metadata, signed_requests, fun(X) -> to_boolean(X) end),
                            ?xpath_text("/md:EntityDescriptor/md:IDPSSODescriptor/md:NameIDFormat/text()",
                                        esaml_idp_metadata, name_format, fun nameid_map/1),
                            ?xpath_text("/md:EntityDescriptor/md:IDPSSODescriptor/md:KeyDescriptor[@use='signing']/ds:KeyInfo/ds:X509Data/ds:X509Certificate/text()", esaml_idp_metadata, certificate, fun(X) -> list_to_binary(X) end),
...
to_xml(#esaml_authnreq{version = V
                      ,issue_instant = Time
                      ,destination = Dest
                      ,issuer = Issuer
                      ,name_format = Format, consumer_location = Consumer
                      }) ->
