...
sign(ElementIn, PrivateKey = #'RSAPrivateKey'{}, CertBin, SigMethod) when is_binary(CertBin) ->
    ...
    SigElem = #xmlElement{name = 'ds:Signature'
                         ,attributes = [#xmlAttribute{name = 'xmlns:ds'
                         ...
                         ,content = [SigInfo
                                    ...
                                    ,#xmlElement{name = 'ds:KeyInfo'
                                                ,content = [#xmlElement{name = 'ds:X509Data'
                                                                       ,content = [#xmlElement{name = 'ds:X509Certificate'
    ...
    Element#xmlElement{content = [esaml_util:build_nsinfo(Ns, SigElem) | Element#xmlElement.content]}.
...
verify(Element, Fingerprints) ->
    ...
    X1 = xmerl_xpath:string("ds:Signature/ds:SignedInfo/ds:SignatureMethod/@Algorithm", Element, [{namespace, DsNs}]),
    [#xmlAttribute{value = SignatureMethodAlgorithm}] = X1,
    {HashFunction, _, _} = signature_props(SignatureMethodAlgorithm),

    Y1 = xmerl_xpath:string("ds:Signature/ds:SignedInfo/ds:CanonicalizationMethod/@Algorithm", Element, [{namespace, DsNs}]),
    [#xmlAttribute{value = "http://www.w3.org/2001/10/xml-exc-c14n#"}] = Y1,
